...
func (a *App) deleteResponseResource(w http.ResponseWriter, r *http.Request, g *gatewayIdentity, id string) error {
	...
	if !stored {
		if taskID != "" {
			...
			deletion.Source = responseBinding{AccountID: task.Selection.Account.ID, Target: task.Target, Items: task.Items, MCPTool: task.MCPTool, CodeTool: task.CodeTool, Containers: task.Containers, VectorStores: task.VectorStores}
		} else {
