...
func testMessagesChat(t *testing.T, a *App, admin string) {
	...
	group := func(platform string) int64 {
		return id(must("POST", "/api/v1/admin/groups", admin, map[string]any{"name": "Messages Chat " + platform, "platform": platform, "rate_multiplier": 2, "model_pricing": []any{price(platform)}}))
	}
