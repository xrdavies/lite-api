...
- 创建带 namespace 的 Gemini Responses 工具调用，再强杀应用/Redis/PostgreSQL；原卷恢复后检查幂等重放不重发、同 Key 续接原样携带函数签名、跨 Key 拒绝及精确账务。这部分只在当前版本执行，不交给缺少签名历史支持的回退版本。
...
Responses→Gemini 的加密工具历史保存 `extra_content.google.thought_signature`，不改变 SQL schema。旧历史没有签名时仍使用既有兼容占位；旧版本会忽略新增元数据，因此回退前须结束依赖 Gemini 工具历史的会话，或由支持签名续传的版本继续处理，不能认为旧版本保持相同续接语义。
